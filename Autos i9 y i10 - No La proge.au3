...
;~ 			FileWriteLine(@ScriptDir &"\temp.html",$mString_HtmlInicio)
			...
			$array = _StringBetween($mString_HtmlInicio,'<div id=grillaavisos>', '</div></a></div>') ;; Para I10
			If UBound ($array) = 0 Then
				$array = _StringBetween($mString_HtmlInicio,'<div id="grillaavisos">', '</div><h3></h3></a><h3></h3></div>') ;; Para I9
			EndIf
...
;~ 			   If UBound($arrayHtml) > 0 Then
;~ 					FileWriteLine($mFileDestino,prosURL($arrayHtml[0]))
;~ 					FileWriteLine(@ScriptDir &"\Registro_" & StringReplace( _nowdate(),'/', '-') & ".txt",$arrayHtml[0])
;~ 				Else
;~ 					$array[$i2] = StringRegExpReplace($array[$i2],"<(.*?)>","")

;~ 					if StringInStr($array[$i2],"Tel:") > 0 Or StringInStr($array[$i2],"Tel.") > 0 Then FileWriteLine($mFileDestino,$array[$i2] & " <-----------------------------")
;~ 					Tempo(5)
;~ 					FileWriteLine(@ScriptDir &"\Registro_" & StringReplace( _nowdate(),'/', '-') & ".txt",$array[$i2])
;~ 				EndIf
	...
	Func importarOrigen ($Lbl_Importar, $pPrB_Descarga, $pPrB_Pros, $pLbl_Destino, $pComB_Asta, $pLbl_Anuncios, $pLbl_ProgPag)
		...
		EndIf
			...
			While $xSalir <= 0
				...
				$xSubString = _StringBetween($mString_HtmlInicio, '<div id="PagerDos">', '</div>')
				$xSubString = _StringBetween($xSubString[0], '<a', '/a>')

				for $i = 0 to UBound($xSubString) -1 Step 1


					if StringInStr($xSubString[$i],">" &  $xInicio &"<") > 0 Then

						$xId = StringRegExp($xSubString[$i],'"(.*?)"' ,1)

						$i = UBound($xSubString)
					ElseIf StringInStr($xSubString[$i],">...<") > 0 And $i > 7 Then
						$xCon = $xCon + 9
						$xId = StringRegExp($xSubString[$i],'id="(.*?)"' ,1)
					EndIf
				Next
				...
				$LinkSigiente = _IEGetObjById($html_Format,$xId[0])
				_IEAction($LinkSigiente, "click")
				$xSubString = $xSubString[UBound($xSubString) - 1]

				While StringInStr($mString_HtmlInicio, $xSubString) > 0 AND $xSalir = 0
					Tempo(20)

					$mString_HtmlInicio =  _IEDocReadHTML($mPag_PagInicio);Inicia el Componente
				WEnd

				IF StringInStr($mString_HtmlInicio, "Pag: "&$xInicio&" de") > 0 Then $xSalir = True
